...
//-----------------------------------------------------------
// Known issues
// 	1. MST1 keep reads same address. MST5 keep writes same address. They
// access same bank different row. 	   Even when TIME_OUT passed, MST5 keep
// get scheduling. MST5 keep send WR cmd.  The bank is not PREable. Even when
// AR_priority high, MST1 can not PRE, tWR is not 0. 	   This is bug.  This
// case never occurs in experiment. FIXME
//-----------------------------------------------------------
#include <algorithm>
#include <assert.h>
#include <iostream>
...
CSLV::CSLV(string cName, int nMemCh) {
  ...
  for (int i = 0; i < BANK_NUM; i++) {
    this->prepare_Refresing[i] = false;
  }
...
EResultType CSLV::Do_AR_fwd(int64_t nCycle) {
  ...
  if (eType == ETRANS_TYPE_FIRST_PTW or eType == ETRANS_TYPE_SECOND_PTW or eType == ETRANS_TYPE_THIRD_PTW or
      eType == ETRANS_TYPE_FOURTH_PTW or eType == ETRANS_TYPE_FIRST_PF or eType == ETRANS_TYPE_SECOND_PF or
      eType == ETRANS_TYPE_THIRD_PF or eType == ETRANS_TYPE_FIRST_RPTW or eType == ETRANS_TYPE_SECOND_RPTW or
      eType == ETRANS_TYPE_THIRD_RPTW or eType == ETRANS_TYPE_FIRST_APTW or eType == ETRANS_TYPE_SECOND_APTW or
      eType == ETRANS_TYPE_THIRD_APTW or eType == ETRANS_TYPE_FOURTH_APTW) {
    this->nPTW++;
  };
...
// upB_new->cpB->GetName().c_str()); this->cpFIFO_B->Display();
...
#ifdef DEBUG_SLV
// printf("[Cycle %3ld: SLV.Do_AR_fwd_MC_Frontend] (%s) access to 0x%lx, put
...
// cpAR->GetName().c_str()); this->cpQ_AR->Display();
#endif
...
#ifdef DEBUG_SLV
// printf("[Cycle %3ld: SLV.Do_AR_fwd_MC_Backend_Response] R data ready bank %d,
...
#ifdef DEBUG_SLV
// printf("[Cycle %3ld: SLV.Do_AW_fwd_MC_Frontend] (%s) access to 0x%lx, put
...
// cpAW->GetName().c_str()); this->cpQ_AW->Display();
#endif
...
#ifdef DEBUG_SLV
// printf("[Cycle %3ld: SLV.Do_Ax_fwd_MC_Backend_Request] PRE/REF command
// issued.\n", nCycle);
#endif
...
// Schedule
  ...
  if (eMemCmd != EMEM_CMD_TYPE_NOP) {
...
#ifdef DEBUG_SLV
      printf("[Cycle %3ld: SLV.Do_Ax_fwd_MC_Backend_Request] MC cmd %s "
             "(%s)(Bank %d, Row 0x%x) sent to bank %d, row 0x%x \n",
             nCycle, cCmd.c_str(), spScheduledMUD->upData->cpAR->GetName().c_str(), this->spMemCmdPkt->nBank,
             this->spMemCmdPkt->nRow, this->spMemCmdPkt->nBank,
             cpMem->cpBank[this->spMemCmdPkt->nBank]->GetActivatedRow());
#endif

      // Stat
      if (eMemCmd == EMEM_CMD_TYPE_ACT) {
        this->nACT_cmd++;
      };
      if (eMemCmd == EMEM_CMD_TYPE_PRE) {
        this->nPRE_cmd++;
      };
    ...
    if (spScheduledMUD != NULL and spScheduledMUD->eUDType == EUD_TYPE_AW) {
      ...
      printf("[Cycle %3ld: SLV.Do_Ax_fwd_MC_Backend_Request] MC cmd %s "
             "(%s)(Bank %d, Row 0x%x) sent to bank %d, row 0x%x \n",
             nCycle, cCmd.c_str(), spScheduledMUD->upData->cpAW->GetName().c_str(), this->spMemCmdPkt->nBank,
             this->spMemCmdPkt->nRow, this->spMemCmdPkt->nBank,
             cpMem->cpBank[this->spMemCmdPkt->nBank]->GetActivatedRow());
#endif
...
#ifdef DEBUG_SLV
// printf("[Cycle %3ld: SLV.Do_Ax_fwd_MC_Backend_Request] %d cycles wait req
// Q_AR scheduled (%s) \n", nCycle, spScheduledMUD->nCycle_wait,
// spScheduledMUD->upData->cpAR->GetName().c_str());
#endif
...
// nCycle, upAR_new->cpAR->GetName().c_str()); this->cpQ_AR->Display();
#endif
...
// nCycle, upAR_new->cpAR->GetName().c_str()); this->cpFIFO_AR->Display();
// this->cpFIFO_AR->CheckFIFO();
#endif
...
// printf("[Cycle %3ld: SLV.Do_Ax_fwd_MC_Backend_Request] %d cycles wait req
// Q_AW scheduled (%s) \n", nCycle, spScheduledMUD->nCycle_wait,
// spScheduledMUD->upData->cpAW->GetName().c_str());
#endif
...
// nCycle, upAW_new->cpAW->GetName().c_str()); this->cpQ_AW->Display();
#endif
...
// nCycle, upAW_new->cpAW->GetName().c_str()); this->cpFIFO_AW->Display();
...
#ifdef DEBUG_SLV
// printf("[Cycle %3ld: SLV.Do_AW_fwd_MC_Backend_Response] W data (%s) ready
// bank %d, row 0x%x.\n", nCycle,
// this->cpFIFO_AW->GetTop()->cpAW->GetName().c_str(),  nBank,
// this->spMemStatePkt->nActivatedRow[nBank]);
#endif
...
// nCycle, upB_new->cpB->GetName().c_str()); this->cpFIFO_B->Display();
#endif
...
EResultType CSLV::Do_R_fwd(int64_t nCycle) {
  ...
  if (cpR_new->IsLast() == ERESULT_TYPE_YES) {
    ...
    // cpR_new->GetName().c_str(), cpR_new->GetID());
  }
// cpR_new->Display();
#endif
...
// cpB_new->GetName().c_str(), cpB_new->GetID());
//  cpB_new->Display();
#endif
...
EResultType CSLV::Handle_PRE_and_REF(int64_t nCycle) {
  ...
  if (this->previous_nBank != -1) {
          ...
          if (spMemStatePkt->forced_PRE[this->previous_nBank] ==
ERESULT_TYPE_YES) {
                  ...
                  printf("[Cycle %3ld: SLV.Do_Ax_fwd_MC_Backend_Request] MC cmd
%s (Bank %d, Row 0x%x) sent to bank %d, row 0x%x \n", nCycle, cCmd.c_str(),
spMemCmdPkt->nBank, spMemCmdPkt->nRow, spMemCmdPkt->nBank,
cpMem->cpBank[spMemCmdPkt->nBank]->GetActivatedRow()); #endif
          ...
          if ( spMemStatePkt->forced_REFI[this->previous_nBank] ==
ERESULT_TYPE_YES) {

                  this->need_Refresing.push_back(this->previous_nBank);

                  if (spMemStatePkt->IsPRE_ready[this->previous_nBank] ==
ERESULT_TYPE_YES) { // Ready for PRE
                          // Issue PRE first because the
                          spMemCmdPkt->eMemCmd = EMEM_CMD_TYPE_PRE;
                          spMemCmdPkt->nBank = this->previous_nBank;
                          spMemCmdPkt->nRow  = 0; // PRE and REFpb: Row is don't
care this->SetMemCmdPkt(spMemCmdPkt); this->cpMem->SetMemCmdPkt(spMemCmdPkt);

                          #ifdef DEBUG_SLV
                          string cCmd =
Convert_eMemCmd2string(spMemCmdPkt->eMemCmd); printf("[Cycle %3ld:
SLV.Do_Ax_fwd_MC_Backend_Request] MC cmd %s (Bank %d, Row 0x%x) sent to bank %d,
row 0x%x \n", nCycle, cCmd.c_str(), spMemCmdPkt->nBank, spMemCmdPkt->nRow,
spMemCmdPkt->nBank, cpMem->cpBank[spMemCmdPkt->nBank]->GetActivatedRow());
                          #endif
  ...
  if (spMemStatePkt->forced_REFI[element] 	== ERESULT_TYPE_YES) {
                  ...
                  if (spMemStatePkt->IsPRE_ready[element] == ERESULT_TYPE_YES) {
// Ready for PRE
                          // Issue PRE first because the
                          spMemCmdPkt->eMemCmd = EMEM_CMD_TYPE_PRE;
                          spMemCmdPkt->nBank 	 = element;
                          spMemCmdPkt->nRow  	 = 0; // PRE and REFpb: Row is
don't care this->SetMemCmdPkt(spMemCmdPkt);
                          this->cpMem->SetMemCmdPkt(spMemCmdPkt);
...
this->need_Refresing.erase(find(this->need_Refresing.begin(),
this->need_Refresing.end(), element)); } if (this->previous_nBank == element) {
this->previous_nBank = -1; } return (ERESULT_TYPE_SUCCESS);
                  }
...
#ifdef DEBUG_SLV
        string cCmd = Convert_eMemCmd2string(this->spMemCmdPkt->eMemCmd);
        printf("[Cycle %3ld: SLV.Do_Ax_fwd_MC_Backend_Request] MC cmd %s (Bank "
               "%d, Row 0x%x) sent to bank %d, row 0x%x \n",
               nCycle, cCmd.c_str(), this->spMemCmdPkt->nBank, this->spMemCmdPkt->nRow, this->spMemCmdPkt->nBank,
               cpMem->cpBank[this->spMemCmdPkt->nBank]->GetActivatedRow());
#endif
...
EResultType CSLV::UpdateState(int64_t nCycle) {
  ...
  if (this->nMax_Q_AR_Wait >= STARVATION_CYCLE) {
    ...
    // printf("[Cycle %3ld: SLV.UpdateState] AR starvation occurs (%d
  ...
  if (this->nMax_Q_AW_Wait >= STARVATION_CYCLE) {
    ...
    // printf("[Cycle %3ld: SLV.UpdateState] AW starvation occurs (%d
...
// Debug
EResultType CSLV::CheckSLV() { return (ERESULT_TYPE_SUCCESS); };
...
EResultType CSLV::PrintStat(int64_t nCycle, FILE *fp) {
  ...
  // (float)(this->nRD_cmd + this->nWR_cmd)/(this->nACT_cmd + this->nPRE_cmd +
  // this->nRD_cmd + this->nWR_cmd));
  printf("\t Bank hit rate                                : %1.2f\n\n",
         (float)(this->nRD_cmd + this->nWR_cmd) / (this->nPRE_cmd + this->nRD_cmd + this->nWR_cmd));

#ifdef STAT_DETAIL
  // printf("\t Empty req Q AR cycles                     : %d\n",
  // this->nEmpty_Q_AR_cycles); printf("\t Empty req Q AW cycles : %d\n",
  // this->nEmpty_Q_AW_cycles);
...
//------------------------------------------------------------------
  ...
  // this->nEmpty_Q_AR_cycles); fprintf(fp, "\t Empty req Q AW cycles : %d\n",
  // this->nEmpty_Q_AW_cycles);
  ...
  // this->nMax_Q_AR_Wait); fprintf(fp, "\t Max req Q all AW waiting cycles :
  // %d\n",	this->nMax_Q_AW_Wait);
