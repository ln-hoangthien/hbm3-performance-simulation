...
// DUONGTRAN add for RangeBuddy
  ...
  cpBuddy->Do_Allocate(MST1_AR_VSTART >> 12, NUM_REQ_PAGES * 2); // 2 image
...
// cpMMU0->anchorDistance = cpBuddy->anchorDistance;	//DUONGTRAN add
  ...
  // cpCacheL3->START_VPN = MST1_AR_VSTART;
  ...
  cpMMU1->Set_START_VPN(MST1_AR_VSTART >> 12); // DUONGTRAN all of MST and MMU share the same page table
  cpMMU2->Set_START_VPN(MST1_AR_VSTART >> 12);
  cpMMU3->Set_START_VPN(MST1_AR_VSTART >> 12);
...
// cpMMU3->Do_B_bwd(nCycle);    DUONGTRAN comment
    ...
    if ( // cpMST0->IsAWTransFinished() == ERESULT_TYPE_YES and
        (cpMST1->IsARTransFinished() == ERESULT_TYPE_YES && cpMST2->IsAWTransFinished() == ERESULT_TYPE_YES &&
         cpMST3->IsARTransFinished() == ERESULT_TYPE_YES)
        // || (cpSLV->nAR == (cpMST1->nARTrans + cpMST3->nARTrans) &&
        // cpSLV->nAW == cpMST2->nAWTrans)
    ) {
...
// Get total avg TLB hit rate and Total PTW
#ifdef MMU_ON
      int Total_nAR_SI = cpMMU1->Get_nAR_SI() + cpMMU2->Get_nAR_SI() + cpMMU3->Get_nAR_SI();
      // int Total_nAR_SI = cpMMU0->Get_nAR_SI() + cpMMU3->Get_nAR_SI();
      int Total_nAW_SI = cpMMU1->Get_nAW_SI() + cpMMU2->Get_nAW_SI() + cpMMU3->Get_nAW_SI();
      // int Total_nAW_SI = cpMMU0->Get_nAW_SI()  + cpMMU3->Get_nAW_SI();
      int Total_nHit_AR_TLB = cpMMU1->Get_nHit_AR_TLB() + cpMMU2->Get_nHit_AR_TLB() + cpMMU3->Get_nHit_AR_TLB();
      // int Total_nHit_AR_TLB = cpMMU0->Get_nHit_AR_TLB() +
      // cpMMU3->Get_nHit_AR_TLB();
      int Total_nHit_AW_TLB = cpMMU1->Get_nHit_AW_TLB() + cpMMU2->Get_nHit_AW_TLB() + cpMMU3->Get_nHit_AW_TLB();
      ...
      // Get TLB reach
      float Avg_TLB_reach =
          (float)(cpMMU1->GetTLB_reach(nCycle) + cpMMU2->GetTLB_reach(nCycle) + cpMMU3->GetTLB_reach(nCycle)) / 3;
      // float Avg_TLB_reach = (float) (cpMMU0->GetTLB_reach(nCycle) +
      // cpMMU3->GetTLB_reach(nCycle)) / 2;

      int total_nPTW = cpMMU1->nPTW_total + cpMMU2->nPTW_total + cpMMU3->nPTW_total + cpMMU1->nAPTW_total_AT +
                       cpMMU2->nAPTW_total_AT + cpMMU3->nAPTW_total_AT;
