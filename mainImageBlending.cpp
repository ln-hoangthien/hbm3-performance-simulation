  ...
  cpSLV->CheckLink();
	...
	while (1) {
		...
		if (//cpMST0->IsAWTransFinished() == ERESULT_TYPE_YES and 
			...
			// Get TLB reach
			float Avg_TLB_reach = (float) (cpMMU1->GetTLB_reach(nCycle) + cpMMU2->GetTLB_reach(nCycle) + cpMMU3->GetTLB_reach(nCycle)) / 3;
			//float Avg_TLB_reach = (float) (cpMMU0->GetTLB_reach(nCycle) + cpMMU3->GetTLB_reach(nCycle)) / 2;


			int total_nPTW = cpMMU1->nPTW_total + cpMMU2->nPTW_total + cpMMU3->nPTW_total + cpMMU1->nAPTW_total_AT + cpMMU2->nAPTW_total_AT + cpMMU3->nAPTW_total_AT;
