...
//-------------------------------------------------------------------------------
#define NUM_TILE_IN_ROW                                                                                                \
  ((int)(ceilf((float)IMG_HORIZONTAL_SIZE / (float)TILEH))) // Number of tiles in image horizontal row
#define NUM_TILE_IN_COL                                                                                                \
  ((int)(ceilf((float)IMG_VERTICAL_SIZE / (float)TILEV))) // Number of tiles in image vertical column
#define TOTAL_NUM_TILE                                                                                                 \
  ((int)(ceilf((float)NUM_TILE_IN_COL * (float)IMG_HORIZONTAL_SIZE /                                                   \
               (float)TILEH))) // Total number of tiles. ImgV maybe unaligned with TILEV
...
// #define HT_PLUS_LEVEL		1
// // Number of levels #define HT_PLUS_ENABLE
...
// 	2. "hta+" algorithm (any)
//-------------------------------------------------------------------------------
// #define HTA_MINUS_ENABLE
// // Assume ImgH aligned hTILEH #define HTA_PLUS_ENABLE
// // ImgH any
...
#define TOTAL_NUM_hTILE                                                                                                \
  (TOTAL_NUM_GROWNUM - (TOTAL_NUM_GROWNUM % hTILEN)) // Total number of granular rows our technique applies to

//-------------------------------------------------------------------------------
// Hierarchical tile dimension
//	"hta+"
//-------------------------------------------------------------------------------
#define NUM_hTILE_ROW                                                                                                  \
  ((int)(ceilf((float)IMG_HORIZONTAL_SIZE / (float)hTILEH))) // Total number of hTILEs in horizontal row
...
// #define AUTO_BFAM_ENABLE
// // Auto map selection. If BFAM condition true, force BFAM even when LIAM
// chosen
...
public:
  ...
  EResultType Set_Block(int A, int B, int Asize,
                        int Bsize);                // (A, B), (Asize, Bsize)
  ...
  // BFAM
  int GetBank_BFAM_BF(int nBank); // Get flipped bank number. BANK_FLIP,
                                  // BANK_FLIP_MINUS algorithm
  int GetBank_BFAM_BS_MINUS(int nBank, int nSPN, int nGSPN,
                            int64_t nAddr); // Get shuffled bank number. BANK_SHUFFLE algorithm
...
private:
  ...
  int nSuperPageNum; // Superpage number. Number of rows (in memory) for
                     // super-page
  int TileHBase;
  int TileVBase;
  ...
  // int**	nTileMap;			// Tile map grid
  // [NUM_TILE_IN_COL][NUM_TILE_IN_ROW] int**	nTileMetric;
  // // Tile map metric [NUM_TILE_IN_COL][NUM_TILE_IN_ROW]
