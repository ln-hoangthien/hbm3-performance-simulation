...
UPUD CTracker::Pop(int nID, EUDType eUDType, ECacheStateType eState) {
  ...
  spScan = this->spCUDList_head;
  while (spScan != NULL) {
    if (spScan->nID == nID and spScan->eUDType == eUDType and spScan->eState == eState) {
...
int CTracker::GetCurNum() { return (this->nCurNum); };

// Get size
int CTracker::GetMaxNum() { return (this->nMaxNum); };

// Get max waiting cycles among enties
int CTracker::GetMaxCycleWait() { return (this->nMaxCycleWait); };
...
SPLinkedCUD CTracker::GetNode_PTW() {
  ...
  while (spScan != NULL) {
    spTarget = spScan;
    if (spTarget->eState == ECACHE_STATE_TYPE_FIRST_PTW or spTarget->eState == ECACHE_STATE_TYPE_SECOND_PTW or
        spTarget->eState == ECACHE_STATE_TYPE_SECOND_PTW_DONE or spTarget->eState == ECACHE_STATE_TYPE_THIRD_PTW or
...
SPLinkedCUD CTracker::GetNode_Pair_PTW_VA_AT(int64_t nVA, EUDType eUDType) {
  ...
  while (spScan != NULL) {
    ...
    if (spTarget->eState == ECACHE_STATE_TYPE_FIRST_PTW or spTarget->eState == ECACHE_STATE_TYPE_SECOND_PTW or
        spTarget->eState == ECACHE_STATE_TYPE_SECOND_PTW_DONE or spTarget->eState == ECACHE_STATE_TYPE_THIRD_PTW or
        spTarget->eState == ECACHE_STATE_TYPE_FOURTH_PTW or spTarget->eState == ECACHE_STATE_TYPE_FOURTH_PTW_DONE or
        spTarget->eState == ECACHE_STATE_TYPE_PTW_HIT) {

      // spTarget->eState == ECACHE_STATE_TYPE_FIRST_APTW  or spTarget->eState
      // == ECACHE_STATE_TYPE_SECOND_APTW or spTarget->eState ==
      // ECACHE_STATE_TYPE_FIRST_RPTW or spTarget->eState ==
      // ECACHE_STATE_TYPE_SECOND_RPTW or spTarget->eState ==
...
SPLinkedCUD CTracker::GetNode_Pair_APTW_VA_AT(int64_t nVA, EUDType eUDType) {
  ...
  while (spScan != NULL) {
    ...
    // Check PTW
    if (spTarget->eState == ECACHE_STATE_TYPE_FIRST_APTW or spTarget->eState == ECACHE_STATE_TYPE_SECOND_APTW or
        spTarget->eState == ECACHE_STATE_TYPE_SECOND_APTW_DONE or spTarget->eState == ECACHE_STATE_TYPE_THIRD_APTW or
        spTarget->eState == ECACHE_STATE_TYPE_FOURTH_APTW or spTarget->eState == ECACHE_STATE_TYPE_FOURTH_APTW_DONE) {

      // spTarget->eState == ECACHE_STATE_TYPE_FIRST_PTW  or spTarget->eState ==
      // ECACHE_STATE_TYPE_SECOND_PTW or spTarget->eState ==
      // ECACHE_STATE_TYPE_FIRST_RPTW or spTarget->eState ==
      // ECACHE_STATE_TYPE_SECOND_RPTW or spTarget->eState ==
      // ECACHE_STATE_TYPE_THIRD_RPTW ) {
...
EResultType CTracker::IsIDMatchPTW(int nID, EUDType eUDType) {
  ...
  while (spScan != NULL) {
    ...
    if (spTarget->nID == nID and spTarget->eUDType == eUDType) {
      ...
      if (spTarget->eState == ECACHE_STATE_TYPE_FIRST_PTW or spTarget->eState == ECACHE_STATE_TYPE_SECOND_PTW or
          spTarget->eState == ECACHE_STATE_TYPE_THIRD_PTW or spTarget->eState == ECACHE_STATE_TYPE_FOURTH_PTW) {
        return (ERESULT_TYPE_YES);
      };
    };
    spScan = spScan->spNext;
...
// EResultType CTracker::IsAddrMatch(int64_t nAddr) {		// Be careful
// FIXME Should be VPN.
//
//	SPLinkedCUD spScan;
//	SPLinkedCUD spTarget;
...
EResultType CTracker::IsAnyPTW_Match_VPN(int64_t nVA) {
  ...
  while (spScan != NULL) {
    ...
    if (spTarget->eUDType == EUD_TYPE_AR) {
      // Check any PTW
      if (spTarget->eState == ECACHE_STATE_TYPE_FIRST_PTW or spTarget->eState == ECACHE_STATE_TYPE_SECOND_PTW or
          spTarget->eState == ECACHE_STATE_TYPE_THIRD_PTW or spTarget->eState == ECACHE_STATE_TYPE_FOURTH_PTW or
          spTarget->eState == ECACHE_STATE_TYPE_FOURTH_PTW_DONE or

          spTarget->eState == ECACHE_STATE_TYPE_FIRST_PF or spTarget->eState == ECACHE_STATE_TYPE_SECOND_PF or

          spTarget->eState == ECACHE_STATE_TYPE_FIRST_APTW or spTarget->eState == ECACHE_STATE_TYPE_SECOND_APTW or
          spTarget->eState == ECACHE_STATE_TYPE_THIRD_APTW or spTarget->eState == ECACHE_STATE_TYPE_FOURTH_APTW or
          spTarget->eState == ECACHE_STATE_TYPE_FOURTH_APTW_DONE or

          spTarget->eState == ECACHE_STATE_TYPE_FIRST_RPTW or spTarget->eState == ECACHE_STATE_TYPE_SECOND_RPTW or
          spTarget->eState == ECACHE_STATE_TYPE_THIRD_RPTW) {

        // Check VPN match
        // int64_t nTargetVPN = spTarget->upData->cpAR->GetAddr() >>
        // BIT_4K_PAGE;	// FIXME Can ba PA
      ...
      if (spTarget->eState == ECACHE_STATE_TYPE_FIRST_PTW or spTarget->eState == ECACHE_STATE_TYPE_SECOND_PTW or
        ...
        // Check VPN match
        // int64_t nTargetVPN = spTarget->upData->cpAW->GetAddr() >>
        // BIT_4K_PAGE;	// FIXME Can be PA
...
EResultType CTracker::IsRPTW_sameVPN_Match_RMM(int64_t nAddr) { // VPN aligned
  ...
  while (spScan != NULL) {
    ...
    if (spTarget->eState == ECACHE_STATE_TYPE_FIRST_RPTW or spTarget->eState == ECACHE_STATE_TYPE_SECOND_RPTW or
        spTarget->eState == ECACHE_STATE_TYPE_THIRD_RPTW) {
      if (spTarget->upData->cpAR->GetAddr() == nAddr) {
