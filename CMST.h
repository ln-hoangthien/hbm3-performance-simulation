...
public:
  ...
  EResultType LoadTransfer_Ax_Custom(int64_t nCycle, FILE *fp,
                                     int nLen); // Trace Custom

  EResultType LoadTransfer_AR(int64_t nCycle, string cAddrMap,
                              string cOperation); // cAddrMap (LIAM, BFAM, TILE). cOperation
                                                  // (Rotation, Raster_scan, CNN)
...
private:
  ...
  int simCDlen = 0;             // FIXME: just a temopary variable. should remove and update
                                // the real behavior.
  bool simDataTransfer = false; // FIXME: just a temopary variable. should
                                // remove and update the real behavior.
  bool passDirty = false;       // FIXME: just a temopary variable. should remove and
                                // update the real behavior.
};
