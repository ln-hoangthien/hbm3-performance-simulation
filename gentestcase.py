...
#ARList = ["AR_TILE", "AR_LIAM", "AR_BFAM", "AR_BGFAM", "AR_OIRAM", "AR_MIN_K_UNION", "AR_NEAR_OPTIMAL", "AR_FLATFISH"]
#AWList = ["AW_TILE", "AW_LIAM", "AW_BFAM", "AW_BGFAM", "AW_OIRAM", "AW_MIN_K_UNION", "AW_NEAR_OPTIMAL", "AW_FLATFISH"]
#ARList = ["AR_LIAM", "AR_BFAM", "AR_BGFAM", "AR_OIRAM", "AR_MIN_K_UNION", "AR_NEAR_OPTIMAL", "AR_FLATFISH"]
#AWList = ["AW_LIAM", "AW_BFAM", "AW_BGFAM", "AW_OIRAM", "AW_MIN_K_UNION", "AW_NEAR_OPTIMAL", "AW_FLATFISH"]
...
mainfileList = ["mainImageDisplay.cpp"]

# For 16-banks
#requestList = [(512, 64), (1024, 128), (1536, 192), (2048, 256), (2560, 320), (3072, 384), (3584, 448), (4096, 512), (4608, 576), (5120, 640), (5632, 704), (6144, 768), (6656, 832), (7168, 896), (7680, 960), (8192, 1024), (8704, 1088), (9216, 1152), (9728, 1216), (10240, 1280), (10752, 1344), (11264, 1408), (11776, 1472), (12288, 1536), (12800, 1600), (13312, 1664), (13824, 1728), (14336, 1792), (14848, 1856), (15360, 1920), (15872, 1984), (16384, 2048)]
#requestList = [(16896,2112), (17408,2176), (17920,2240), (18432,2304), (18944,2368), (19456,2432), (19968,2496), (20480,2560), (20992,2624), (21504,2688), (22016,2752), (22528,2816), (23040,2880), (23552,2944), (24064,3008), (24576,3072)]

# For 32-banks - Reduced set
...
requestList = [(1024, 128), (2048, 256), (3072, 384), (4096, 512), (5120, 640), (6144, 768), (7168, 896), (8192, 1024), (9216, 1152), (10240, 1280), (11264, 1408), (12288, 1536), (13312, 1664), (14336, 1792), (15360, 1920), (16384, 2048)]

# For 48-banks - Reduced set
...
#requestList = [(1536, 192), (3072, 384), (4608, 576), (6144, 768), (7680, 960), (9216, 1152), (10752, 1344), (12288, 1536), (13824, 1728), (15360, 1920), (16896, 2112), (18432, 2304), (19968, 2496), (21504, 2688), (23040, 2880), (24576, 3072)]

# For 64-banks - Reduced set
					...
					for AWType in AWList:
						for BankNum in BankList:
							...
							if (coalescingType == "BBS_TRAD_SELECTED" and (ARType != "AR_LIAM" or AWType != "AW_LIAM")):
								continue
							if (scenarioType == "RASTER_SCAN" 	and (ARType == "AR_BFAM" or ARType == "AR_BFAM" or ARType == "AR_BGFAM" or ARType == "AR_BFFAM"
																	or ARType == "AR_MIN_K_UNION_TILE_COL_WISE" or ARType == "AR_MIN_K_UNION_COL_WISE"  or ARType == "AR_MIN_K_UNION_TILE_ONLY" 
							...
							if (scenarioType == "TILE" 			and (ARType == "AR_BFAM" or ARType == "AR_BFAM" or ARType == "AR_BGFAM" or ARType == "AR_BFFAM"
								...
								continue
							#if ((scenarioType != "TILE") and (tileSize > 1)):
							#	continue
							if ((scenarioType == "TILE" or ("TILE" in ARType)) and (tileSize == 1)):
								continue
							if ((scenarioType == "TILE") and (mainfile != "mainImageDisplay.cpp" and mainfile != "mainImagePreview.cpp")):
								continue
							...
							for blockSize in blockList:
								for requestSize in requestList:
									...
									testcaseLine += "make" + "\t" + coalescingType + "\t" + "AR=" + ARType + "\t" + "AW=" + AWType + "\t" + "BANK_NUM=" + str(BankNum) + "\t" + "SCENARIO=" + scenarioType + "\t" + "BLOCKSIZE_INIT=" + str(blockSize) + "\t" + "IMG_HORIZONTAL_SIZE=" + str(requestSize[0]) + "\t" + "IMG_VERTICAL_SIZE=" + str(requestSize[1]) + "\t" + "MAINFILE=" + mainfile + "\t" + "NUM_TOTAL_PAGES=" + str(nTotalPages) +  "\t" + "TILE_SIZE=" + str(tileSize) + "\t" + "SEED_INIT=" + str(SEED_INIT) +  "; "
									outFile = coalescingType + "_" + (mainfile.replace(".cpp", "_")).replace("main", "") + scenarioType.replace("_", "") + "_" + str(ARType) + "_" + str(AWType) + "_" + str(BankNum) + "_" + str(blockSize) + "_" + str(requestSize[0]) + "_" + str(requestSize[1]) + "_" + str(tileSize)
