...
#define MST1_AR_VSTART MST0_AW_VSTART
#define MST2_AW_VSTART (MST1_AR_VSTART + IMG_HORIZONTAL_SIZE * IMG_VERTICAL_SIZE * BYTE_PER_PIXEL)
// #define MST2_AW_VSTART 0x0000000600000000
#define MST3_AR_VSTART MST2_AW_VSTART
...
// DUONGTRAN add for RangeBuddy
  ...
  cpBuddy->Do_Allocate(MST0_AW_VSTART >> 12, NUM_REQ_PAGES * 2); // 2 images
  ...
  // Allocate
  NUM_REQ_PAGES = IMG_HORIZONTAL_SIZE * IMG_VERTICAL_SIZE * BYTE_PER_PIXEL / 4096;
  // cpBuddy->Do_Allocate(MST1_AR_VSTART >> 12, NUM_REQ_PAGES);

  // Get page-table
  spPageTable = cpBuddy->Get_PageTable();

  // Set page-table
  cpMMU1->Set_PageTable(spPageTable);

#ifdef AT_ENABLE
  cpMMU1->anchorDistance = cpBuddy->anchorDistance; // DUONGTRAN add
#endif
  ...
  NUM_REQ_PAGES = IMG_HORIZONTAL_SIZE * IMG_VERTICAL_SIZE * BYTE_PER_PIXEL / 4096;
  assert(NUM_TOTAL_PAGES >= NUM_REQ_PAGES);
  // cpBuddy->Do_Allocate(MST2_AW_VSTART >> 12, NUM_REQ_PAGES);

  // Get page-table
  spPageTable = cpBuddy->Get_PageTable();

  // Set page-table
  cpMMU2->Set_PageTable(spPageTable);

#ifdef AT_ENABLE
  cpMMU2->anchorDistance = cpBuddy->anchorDistance; // DUONGTRAN add
#endif
  ...
  // cpCacheL3->START_VPN = MST0_AW_VSTART;
  ...
  // cpMST2->Set_nAR_START_ADDR(MST2_AR_VSTART);
  cpMST2->Set_nAW_START_ADDR(MST2_AW_VSTART);
  ...
  // cpMMU1->Set_nAR_START_ADDR(MST1_AR_VSTART);
  // cpMMU1->Set_nAW_START_ADDR(MST1_AW_VSTART);

  // cpMMU2->Set_nAR_START_ADDR(MST2_AR_VSTART);
  // cpMMU2->Set_nAW_START_ADDR(MST2_AW_VSTART);
  ...
  cpMMU0->Set_START_VPN(MST0_AW_VSTART >> 12); // DUONGTRAN all of MST and MMU share the same page table
  cpMMU1->Set_START_VPN(MST0_AW_VSTART >> 12);
  cpMMU2->Set_START_VPN(MST0_AW_VSTART >> 12);
  cpMMU3->Set_START_VPN(MST0_AW_VSTART >> 12);
  ...
  ////------------------------------
  cpMST0->Set_AR_ISSUE_MIN_INTERVAL(4);
  cpMST0->Set_AW_ISSUE_MIN_INTERVAL(4);

  cpMST1->Set_AR_ISSUE_MIN_INTERVAL(4);
  cpMST1->Set_AW_ISSUE_MIN_INTERVAL(4);

  cpMST2->Set_AR_ISSUE_MIN_INTERVAL(4);
  cpMST2->Set_AW_ISSUE_MIN_INTERVAL(4);

  cpMST3->Set_AR_ISSUE_MIN_INTERVAL(4);
  cpMST3->Set_AW_ISSUE_MIN_INTERVAL(4);
...
// cpMMU3->Do_W_bwd(nCycle);
    ...
    // cpMST0->Do_R_bwd(nCycle);   // DUONGTRAN comment
...
// cpMMU3->Do_B_bwd(nCycle);    DUONGTRAN comment
    ...
    if ((cpMST0->IsAWTransFinished() == ERESULT_TYPE_YES && cpMST1->IsARTransFinished() == ERESULT_TYPE_YES &&
         cpMST2->IsAWTransFinished() == ERESULT_TYPE_YES && cpMST3->IsARTransFinished() == ERESULT_TYPE_YES)
...
// Get total avg TLB hit rate and PTW
      ...
      int Total_nHit_AR_TLB =
          cpMMU0->Get_nHit_AR_TLB() + cpMMU1->Get_nHit_AR_TLB() + cpMMU2->Get_nHit_AR_TLB() + cpMMU3->Get_nHit_AR_TLB();
      // int Total_nHit_AR_TLB = cpMMU0->Get_nHit_AR_TLB() +
      // cpMMU3->Get_nHit_AR_TLB();
      int Total_nHit_AW_TLB =
          cpMMU0->Get_nHit_AW_TLB() + cpMMU1->Get_nHit_AW_TLB() + cpMMU2->Get_nHit_AW_TLB() + cpMMU3->Get_nHit_AW_TLB();
      ...
      // Get TLB reach
      float Avg_TLB_reach = (float)(cpMMU0->GetTLB_reach(nCycle) + cpMMU1->GetTLB_reach(nCycle) +
                                    cpMMU2->GetTLB_reach(nCycle) + cpMMU3->GetTLB_reach(nCycle)) /
                            4;
      // float Avg_TLB_reach = (float) (cpMMU0->GetTLB_reach(nCycle) +
      // cpMMU3->GetTLB_reach(nCycle)) / 2;

      int total_nPTW = cpMMU0->nPTW_total + cpMMU1->nPTW_total + cpMMU2->nPTW_total + cpMMU3->nPTW_total +
                       cpMMU0->nAPTW_total_AT + cpMMU1->nAPTW_total_AT + cpMMU2->nAPTW_total_AT +
                       cpMMU3->nAPTW_total_AT;
...
// cpBuddy->Do_Deallocate(MST1_AR_VSTART >> 12);
// cpBuddy->Do_Deallocate(MST2_AW_VSTART >> 12);
