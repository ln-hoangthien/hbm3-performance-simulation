...
CMem::CMem(string cName) {
  ...
  for (int i = 0; i < BANK_NUM / BanksPerBGroup; i++)
    this->nCnt_CCD[i] = -1; // Bank Group. Column-to-Column delay. RD2RD. WR2WR
  for (int i = 0; i < BANK_NUM / BanksPerBGroup; i++)
    this->nCnt_RRD[i] = -1; // Bank Group. ACT2ACT
  for (int i = 0; i < BANK_NUM / BanksPerBGroup; i++)
    this->nCnt_WTR[i] = -1; // Bank Group. WR2RD
...
EResultType CMem::Reset() {
  ...
  // Initialize cnt
  for (int i = 0; i < BANK_NUM / BanksPerBGroup; i++)
    this->nCnt_CCD[i] = tCCD; // Global. RD2RD. WR2WR
  for (int i = 0; i < BANK_NUM / BanksPerBGroup; i++)
    this->nCnt_RRD[i] = tRRD; // Global. ACT2ACT diff bank
  for (int i = 0; i < BANK_NUM / BanksPerBGroup; i++)
    this->nCnt_WTR[i] = tWTR; // Global. WR2RD diff bank
...
// Get mem name
string CMem::GetName() { return (this->cName); };
...
EResultType CMem::IsWR_global_ready(int bank) {
  for (int nbank = 0; nbank < BANK_NUM; nbank++) {
    int check_bankgroup = std::floor(nbank / BanksPerBGroup);
    if (this->nCnt_CCD[check_bankgroup] < Get_tCCD(bank, nbank) or this->nCnt_RTW[check_bankgroup] < tRTW)
      return (ERESULT_TYPE_NO);
  };
  return (ERESULT_TYPE_YES);
};
...
EResultType CMem::IsACT_global_ready(int bank) {
  for (int nbank = 0; nbank < BANK_NUM; nbank++) {
    ...
    if (this->nCnt_RRD[check_bankgroup] < Get_tRRD(bank, nbank) or this->nCnt_RREFD < tRREFD)
...
EResultType CMem::UpdateState() {
  ...
  if (this->spMemCmdPkt->eMemCmd == EMEM_CMD_TYPE_ACT) {
    ...
    for (int i = 0; i < BANK_NUM / BanksPerBGroup; i++)
      this->nCnt_RRD[i]++; // obliviously increase
  };
...
// Debug
EResultType CMem::Display() { return (ERESULT_TYPE_SUCCESS); };
