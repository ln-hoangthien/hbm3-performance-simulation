...
//	(2) Bank number = (RBC bank number) XOR (Least significant k bits of row
// number). Bank k bits.
//-----------------------------------------------------------
// Note
//	(1) RBC, RCBC, BRC, pRBC, pRCBC do not support multi channel.
//	(2) We implemented other memory maps (RCBC, pRCBC, BRC). See Top.cpp in
// IEEE access 2019 submitted paper.
//-----------------------------------------------------------
// Multi channel
...
int GetBankNum_AMap_Global(int64_t nAddr) {
  ...
  // int nBank = ( (unsigned) (nBank_tmp << (32-BANK_WIDTH)) >>
  // (32-BANK_WIDTH));	// 32-bit VA
  ...
  // int nBank = ( (unsigned) (nBank_tmp << (32-BANK_WIDTH)) >>
  // (32-BANK_WIDTH));	// 32-bit VA
  // nBank        = ( (uint64_t) (nBank_tmp << (64-BANK_WIDTH-nColWidthTmp)) >>
  // (64-BANK_WIDTH-nColWidthTmp));	// 64-bit VA
  nBank = ((uint64_t)(nBank_tmp << (64 - BANK_WIDTH) >> (64 - BANK_WIDTH))); // 64-bit VA
#endif
...
int GetChannelNum_AMap_Global(int64_t nAddr) {
  ...
  // int nCh = ( (unsigned) (nCh_tmp << (32 - MEM_CH_WIDTH)) >> (32 -
  // MEM_CH_WIDTH));	// 32-bit VA
  ...
  // int nCh = ( (unsigned) (nCh_tmp << (32 - MEM_CH_WIDTH)) >> (32 -
  // MEM_CH_WIDTH));	// 32-bit VA int nCh = ( (uint64_t) (nCh_tmp << (64 -
  // MEM_CH_WIDTH)) >> (64 - MEM_CH_WIDTH));	// 64-bit VA

  int nCh = -1;
  if (MEM_CH_WIDTH == 0)
    nCh = nCh_tmp & 0; // 1 ch
  else if (MEM_CH_WIDTH == 1)
    nCh = nCh_tmp & 1; // 2 ch
  else if (MEM_CH_WIDTH == 2)
    nCh = nCh_tmp & 3; // 4 ch
  else if (MEM_CH_WIDTH == 3)
    nCh = nCh_tmp & 7; // 8 ch
  else
    assert(-1);
...
int GetCacheChNum_Global(int64_t nAddr) {
  ...
  // int nCh = ( (unsigned) (nCh_tmp << (32 - CACHE_WIDTH)) >> (32 -
  // CACHE_WIDTH));	// 32-bit VA int nCh = ( (uint64_t) (nCh_tmp << (64 -
  // CACHE_WIDTH)) >> (64 - CACHE_WIDTH));	// 64-bit VA FIXME overflow

  int nCh = -1;
  if (CACHE_WIDTH == 0)
    nCh = nCh_tmp & 0; // 1 ch
  else if (CACHE_WIDTH == 1)
    nCh = nCh_tmp & 1; // 2 ch
  else if (CACHE_WIDTH == 2)
    nCh = nCh_tmp & 3; // 4 ch
  else if (CACHE_WIDTH == 3)
    nCh = nCh_tmp & 7; // 8 ch
  else
    assert(-1);
...
//
//	return	( (uint64_t) nAddr >> (COL_WIDTH + BANK_WIDTH));  // RBC or RCBC
// or pRBC
// };
...
int64_t GetRowNum_MMap_Global(int64_t nAddr) {
  ...
  // uint64_t nRow = ( (uint64_t) (nRow_tmp << (32 - ROW_WIDTH)) >> (32 -
  // ROW_WIDTH));	// Arch32 uint64_t nRow = ( (uint64_t) (nRow_tmp << (64
  // - ROW_WIDTH)) >> (64 - ROW_WIDTH));	// Arch64. FIXME overflow

  int nRow = -1;
  if (ROW_WIDTH == 0)
    nRow = nRow & 0b0; // 2^0 row
  else if (ROW_WIDTH == 1)
    nRow = nRow & 0b1; // 2^1 rows
  else if (ROW_WIDTH == 2)
    nRow = nRow & 0b11; // 2^2 rows
  else if (ROW_WIDTH == 3)
    nRow = nRow & 0b111; // 2^3 rows
  else if (ROW_WIDTH == 4)
    nRow = nRow & 0b1111; // 2^4 rows
  else if (ROW_WIDTH == 5)
    nRow = nRow & 0b11111; // 2^5 rows
  else if (ROW_WIDTH == 6)
    nRow = nRow & 0b111111; // 2^6 rows
  else if (ROW_WIDTH == 7)
    nRow = nRow & 0b1111111; // 2^7 rows
  else if (ROW_WIDTH == 8)
    nRow = nRow & 0b11111111; // 2^8 rows
  else if (ROW_WIDTH == 9)
    nRow = nRow & 0b111111111; // 2^9 rows
  else if (ROW_WIDTH == 10)
    nRow = nRow & 0b1111111111; // 2^10 rows
  else if (ROW_WIDTH == 11)
    nRow = nRow & 0b11111111111; // 2^11 rows
  else if (ROW_WIDTH == 12)
    nRow = nRow & 0b111111111111; // 2^12 rows
  else if (ROW_WIDTH == 13)
    nRow = nRow & 0b1111111111111; // 2^13 rows
  else if (ROW_WIDTH == 14)
    nRow = nRow & 0b11111111111111; // 2^14 rows
  else if (ROW_WIDTH == 15)
    nRow = nRow & 0b111111111111111; // 2^15 rows
  else if (ROW_WIDTH == 16)
    nRow = nRow & 0b1111111111111111; // 2^16 rows
  else if (ROW_WIDTH == 17)
    nRow = nRow & 0b11111111111111111; // 2^17 rows
  else if (ROW_WIDTH == 18)
    nRow = nRow & 0b111111111111111111; // 2^18 rows
  else if (ROW_WIDTH == 19)
    nRow = nRow & 0b1111111111111111111; // 2^19 rows
  else if (ROW_WIDTH == 20)
    nRow = nRow & 0b11111111111111111111; // 2^20 rows
  else if (ROW_WIDTH == 21)
    nRow = nRow & 0b111111111111111111111; // 2^21 rows
  else if (ROW_WIDTH == 22)
    nRow = nRow & 0b1111111111111111111111; // 2^22 rows
  else if (ROW_WIDTH == 23)
    nRow = nRow & 0b11111111111111111111111; // 2^23 rows
  else if (ROW_WIDTH == 24)
    nRow = nRow & 0b111111111111111111111111; // 2^24 rows
  else if (ROW_WIDTH == 25)
    nRow = nRow & 0b1111111111111111111111111; // 2^25 rows
  else if (ROW_WIDTH == 26)
    nRow = nRow & 0b11111111111111111111111111; // 2^26 rows
  else if (ROW_WIDTH == 27)
    nRow = nRow & 0b111111111111111111111111111; // 2^27 rows
  else if (ROW_WIDTH == 28)
    nRow = nRow & 0b1111111111111111111111111111; // 2^28 rows
  else if (ROW_WIDTH == 29)
    nRow = nRow & 0b11111111111111111111111111111; // 2^29 rows
  else if (ROW_WIDTH == 30)
    nRow = nRow & 0b111111111111111111111111111111; // 2^30 rows
  else if (ROW_WIDTH == 31)
    nRow = nRow & 0b1111111111111111111111111111111; // 2^31 rows
  else if (ROW_WIDTH == 32)
    nRow = nRow & 0b11111111111111111111111111111111; // 2^32 rows
  else if (ROW_WIDTH == 33)
    nRow = nRow & 0b111111111111111111111111111111111; // 2^33 rows
  else if (ROW_WIDTH == 34)
    nRow = nRow & 0b1111111111111111111111111111111111; // 2^34 rows
  else if (ROW_WIDTH == 35)
    nRow = nRow & 0b11111111111111111111111111111111111; // 2^35 rows
  else if (ROW_WIDTH == 36)
    nRow = nRow & 0b111111111111111111111111111111111111; // 2^36 rows
  else if (ROW_WIDTH == 37)
    nRow = nRow & 0b1111111111111111111111111111111111111; // 2^37 rows
  else if (ROW_WIDTH == 38)
    nRow = nRow & 0b11111111111111111111111111111111111111; // 2^38 rows
  else if (ROW_WIDTH == 39)
    nRow = nRow & 0b111111111111111111111111111111111111111; // 2^39 rows
  else if (ROW_WIDTH == 40)
    nRow = nRow & 0b1111111111111111111111111111111111111111; // 2^40 rows
  else if (ROW_WIDTH == 41)
    nRow = nRow & 0b11111111111111111111111111111111111111111; // 2^41 rows
  else if (ROW_WIDTH == 42)
    nRow = nRow & 0b111111111111111111111111111111111111111111; // 2^42 rows
  else if (ROW_WIDTH == 43)
    nRow = nRow & 0b1111111111111111111111111111111111111111111; // 2^43 rows
  else if (ROW_WIDTH == 44)
    nRow = nRow & 0b11111111111111111111111111111111111111111111; // 2^44 rows
  else if (ROW_WIDTH == 45)
    nRow = nRow & 0b111111111111111111111111111111111111111111111; // 2^45 rows
  else if (ROW_WIDTH == 46)
    nRow = nRow & 0b1111111111111111111111111111111111111111111111; // 2^46 rows
  else if (ROW_WIDTH == 47)
    nRow = nRow & 0b11111111111111111111111111111111111111111111111; // 2^47 rows
  else if (ROW_WIDTH == 48)
    nRow = nRow & 0b111111111111111111111111111111111111111111111111; // 2^48 rows
  else if (ROW_WIDTH == 49)
    nRow = nRow & 0b1111111111111111111111111111111111111111111111111; // 2^49 rows
  else if (ROW_WIDTH == 50)
    nRow = nRow & 0b11111111111111111111111111111111111111111111111111; // 2^50 rows
  else if (ROW_WIDTH == 51)
    nRow = nRow & 0b111111111111111111111111111111111111111111111111111; // 2^51 rows
  else if (ROW_WIDTH == 52)
    nRow = nRow & 0b1111111111111111111111111111111111111111111111111111; // 2^52 rows
  else if (ROW_WIDTH == 53)
    nRow = nRow & 0b11111111111111111111111111111111111111111111111111111; // 2^53 rows
  else
    assert(-1);
  ...
  // return  ( (uint64_t) nAddr >> (BANK_WIDTH + COL_WIDTH));
  // // RBC, RCBC, pRBC, pRCBC.
...
//	nBank = ( (uint64_t) (nBank_tmp << (64-BANK_WIDTH)) >> (64-BANK_WIDTH));
//// RBC 	#endif
//
//	#ifdef RCBC
...
//	// (4) Bank number = (RBC bank number) XOR (Least significant k bits of
// RBC row number)
//	//------------------------------
//
//	// (1) Get RBC bank number
...
int GetBankNum_MMap_Global(int64_t nAddr) {
...
#ifdef RBC
  uint64_t nBank_tmp = nAddr >> (COL_WIDTH);
  // uint64_t nBank = ( (uint64_t) (nBank_tmp << (32 - BANK_WIDTH)) >> (32 -
  // BANK_WIDTH));	// RBC. Arch32
  ...
  // (2) Get RBC row number
  // (3) Get least significant k bits of RBC row number
  // (4) Bank number = (RBC bank number) XOR (Least significant k bits of RBC
  // row number)
  //------------------------------
  ...
  // (4) Bank number = (RCBC bank number) XOR (Least significant k bits of RCBC
  // row number)
  //------------------------------
  ...
  // uint64_t nBank = ( (unsigned) (nBank_tmp << (32 - MEM_CH_WIDTH -
  // BANK_WIDTH)) >> (32 - MEM_CH_WIDTH - BANK_WIDTH));  // 32-bit VA uint64_t
  // nBank = ( (uint64_t) (nBank_tmp << (64 - MEM_CH_WIDTH - BANK_WIDTH)) >> (64
  // - MEM_CH_WIDTH - BANK_WIDTH));  // 64-bit VA
...
//
//	#ifdef RCBC
//	uint64_t nCol2  = (uint64_t) (nAddr << (64 - COL2_WIDTH)) >> (64 -
// COL2_WIDTH);
//
//	uint64_t nCol1_tmp = nAddr >> (BANK_WIDTH + COL2_WIDTH);
//	uint64_t nCol1 = ( (uint64_t) (nCol1_tmp << (64 - BANK_WIDTH -
// COL2_WIDTH)) >> (64 - BANK_WIDTH - COL2_WIDTH));
//
...
int GetColNum_MMap_Global(int64_t nAddr) {
...
#ifdef RBC
  // int nCol = ( (uint64_t) (nAddr << (32 - COL_WIDTH)) >> (32 - COL_WIDTH));
  // // RBC. Arch32
  ...
  uint64_t nCol1_tmp = nAddr >> (BANK_WIDTH + COL2_WIDTH);
  // uint64_t nCol1 = ( (uint64_t) (nCol1_tmp << (32 - BANK_WIDTH - COL2_WIDTH))
  // >> (32 - BANK_WIDTH - COL2_WIDTH));	// Arch32 uint64_t nCol1 = (
  // (uint64_t) (nCol1_tmp << (64 - BANK_WIDTH - COL2_WIDTH)) >> (64 -
  // BANK_WIDTH - COL2_WIDTH));	// Arch64
  uint64_t nCol1 = ((uint64_t)(nCol1_tmp << (64 - COL1_WIDTH)) >> (64 - COL1_WIDTH)); // Arch64
...
int GetChannelNum_MMap_Global(int64_t nAddr) {
  ...
  uint64_t nCh_tmp = nAddr >> (BANK_WIDTH + COL_WIDTH);
  // int nCh = ( (unsigned) (nCh_tmp << (32 - MEM_CH_WIDTH)) >> (32 -
  // MEM_CH_WIDTH));		// 32-bit VA int nCh = ( (uint64_t) (nCh_tmp <<
  // (64 - MEM_CH_WIDTH)) >> (64 - MEM_CH_WIDTH));		// 64-bit VA
  int nCh = ((uint64_t)(nCh_tmp << (64 - MEM_CH_WIDTH)) >> (64 - MEM_CH_WIDTH)); // 64-bit VA
...
int64_t GetAddr_AMap_Global(int64_t nRow, int nMemCh, int nBank, int nCol) {
  ...
  return (uint64_t)((nRow << (MEM_CH_WIDTH + BANK_WIDTH + COL_WIDTH)) + (nMemCh << (BANK_WIDTH + COL_WIDTH)) +
                    (nBank << COL_WIDTH) + (nCol));
};
