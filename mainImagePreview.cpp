...
int main() {
  ...
  cpSLV->CheckLink();
	...
	// DUONGTRAN add for RangeBuddy
		...
		// Allocate
		NUM_REQ_PAGES = IMG_HORIZONTAL_SIZE*IMG_VERTICAL_SIZE*BYTE_PER_PIXEL/4096;
		//cpBuddy->Do_Allocate(MST3_AR_VSTART >> 12, NUM_REQ_PAGES);     // FIXME using same page table for all MST, MMU
	...
	while (1) {
		...
		#ifdef RASTER_SCAN
			//cpMST0->LoadTransfer_AR(nCycle, cAR_AddrMap, "RASTER_SCAN");  //DUONGTRAN comment
		...
		#elif ROTATION
			//cpMST0->LoadTransfer_AR(nCycle, cAR_AddrMap, "ROTATION");  //DUONGTRAN comment
