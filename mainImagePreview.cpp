...
#define MST3_AR_VSTART MST0_AW_VSTART
...
// DUONGTRAN add for RangeBuddy
  ...
  // Allocate
  NUM_REQ_PAGES = IMG_HORIZONTAL_SIZE * IMG_VERTICAL_SIZE * BYTE_PER_PIXEL / 4096;
  // cpBuddy->Do_Allocate(MST3_AR_VSTART >> 12, NUM_REQ_PAGES);     // FIXME
  // using same page table for all MST, MMU
  ...
  cpMST0->Set_nAR_GEN_NUM(0); // DUONGTRAN comment
...
// Master geneates transactions
#ifdef RASTER_SCAN
    // cpMST0->LoadTransfer_AR(nCycle, cAR_AddrMap, "RASTER_SCAN");  //DUONGTRAN
    // comment
    ...
    cpMST3->LoadTransfer_AR(nCycle, cAR_AddrMap, "RASTER_SCAN");
    // cpMST3->LoadTransfer_AW(nCycle, cAW_AddrMap, "ROTATION");  // DUONGTRAN
    // comment
#else
    assert(0);
#endif
...
//---------------------------
    ...
    //---------------------------------
    if ((cpMST0->IsAWTransFinished() == ERESULT_TYPE_YES && cpMST3->IsARTransFinished() == ERESULT_TYPE_YES)
...
// Get total avg TLB hit rate and PTW
      ...
      int total_nPTW = cpMMU0->nPTW_total + cpMMU3->nPTW_total + cpMMU0->nAPTW_total_AT + cpMMU3->nAPTW_total_AT;
