...
EResultType CFIFO::Push(UPUD upThis, int nLatency) {
  ...
  // printf("CFIFO::Push() is called. FIFO name: %s\n", this->cName.c_str());
...
UPUD CFIFO::Pop() {
  ...
  // printf("CFIFO::Pop() is called. FIFO name: %s\n", this->cName.c_str());
...
int CFIFO::GetCurNum() { return (this->nCurNum); };

// Get size
int CFIFO::GetMaxNum() { return (this->nMaxNum); };
